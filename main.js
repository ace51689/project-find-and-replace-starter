...
replaceAllButton.addEventListener('click', function () {
    let findInputValue = findInput.value.toString()
    let replaceInputValue = replaceInput.value.toString()
    // console.log(findInputValue, replaceInputValue)
    for (let increment = 0; increment < rowElements.length; increment += 1) {
        let result = getCellElements(rowElements[increment])
        // console.log(result)
        for (let count = 0; count < result.length; count += 1) {
            let cellElement = result[count].innerHTML
